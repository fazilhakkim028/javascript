...
(function (factory) {
    ...
    var Comments = {
        ...
        events: {
            ...
            'click li.comment button.loadmore' : 'loadmore',
        ...
        getDefaultOptions: function() {
            return {
                ...
                loadmore:function(success, error) {success(commentArray)},       
                refresh: function() {},       
        ...
        init: function(options, el) {
         ...
         // Detect mobile devices
            ...
            // kre comment and analytics html append
           var kre_top_html='<div class="kre-comments_data"><a href="">'+this.options.comments+'</a></div><div class="kre-comments_count_blk" id="commenters"></div><div class="kre-comments_analystics"><a href="">'+this.options.analytics+'</a></div>';
            this.$el.parents("#kre-comments-parent").find(".kre-comments-top").append(kre_top_html);
    ...
    DowndateCommentModel: function(commentModel) {
            $.extend(this.commentsById[commentModel.id], commentModel);
        },
        render: function() {
            var self = this;
        ...
        createComments: function() {
          ...
          $('> li:last-child',commentList).append(loadmore);
          if(this.options.enableLoadmorebutton==true){$(loadmore).addClass("kreloadshow")}
          $(loadmore).after('<span class="spinner krehide"><i class="fa fa-spinner fa-spin"></i></span>');
        },
        ...
        addComment: function(commentModel, commentList) {
            ...
            // Case: reply
            if(commentModel.parent) {
                var directParentEl = commentList.find('.comment#'+commentModel.parent+'');
        ...
        removeComment: function(commentId) {
            ...
            var commentElements = this.$el.find('li.comment#'+commentModel.id+'');
                ...
                comments.sort(function(commentA, commentB) {
                    ...
                    if(self.options.enableUpvoting) {
                        ...
                        pointsOfB += commentB.upvoteCount;
                         ...
                         if(self.options.enableDownvoting) {
                        pointsOfA += commentA.downvoteCount;
                        pointsOfB += commentB.downvoteCount;
                    }
      ...
      sortUsers: function(users) {
        ...
        sortAndReArrangeComments: function(sortKey) {
            ...
            $(mainLevelComments).each(function(index, commentModel) {
                var commentEl = commentList.find('> li.comment#'+commentModel.id+'');
        ...
        navigationElementClicked: function(ev) {
            ...
            this.showActiveSort();
       if(!(typeof KRE_adv_list=="undefined")||KRE_adv_list.length>0){
...
loadmore:function(){
          var self = this,arr;
            this.arrval=this.getComments();
               // var layload=this.lazyloadarr,arrblock=[];
                // var arr=this.arrval.map(function(jsonval){
              	         // arrblock.push(jsonval);
              // })
arr=this.arrval;
  ...
  //          this.usersById = {};
         ...
         //   this.$el.empty();
          //  this.createHTML();
            ...
            var commentsFetched = function(commentsArray) {
                // Convert comments to custom data model
                commentsArray=arr.concat(commentsArray);
                var commentModels = commentsArray.map(function(commentsJSON){
                    return self.createCommentModel(commentsJSON)
                });
            ...
            this.options.loadmore(commentsFetched, dataFetched);	
},
        deleteComment: function(ev) {
            var self = this;
            var deleteButton = $(ev.currentTarget);
            var commentEl = deleteButton.parents('.comment').first();
            var commentJSON =  $.extend({}, this.commentsById[commentEl.attr('id')]);
         ...
         hashtagClicked: function(ev) {
            var el = $(ev.currentTarget);
            var value = $(ev.target).parents("li").attr("id");
...
// 
           ...
           // commentModel.userHasUpvoted = !commentModel.userHasUpvoted;
            ...
            commentJSON = this.applyExternalMappings(commentJSON);
if($(ev.currentTarget).attr("data-vote")=="true"){
$(ev.currentTarget).siblings(".krevote").attr("disabled", false);
commentJSON["user_has_upvoted"]=false
}
if($(ev.currentTarget).attr("data-vote")=="false"){
	$(ev.currentTarget).siblings(".krevote").attr("disabled", true);
...
if($(ev.currentTarget).attr("data-vote")=="true"){
	$(ev.currentTarget).siblings(".krevote").attr("disabled", false);
commentJSON["user_has_downvoted"]=false
}
if($(ev.currentTarget).attr("data-vote")=="false"){
	$(ev.currentTarget).attr("data-vote","true")
	$(ev.currentTarget).siblings(".krevote").attr("disabled", true);
commentJSON["user_has_downvoted"]=true
}
            var success = function(commentJSON) {
                var commentModel = self.createCommentModel(commentJSON);
                self.DowndateCommentModel(commentModel);
            ...
            var error = function() {
                ...
                // Revert changes
                commentModel.userHasDownvoted = !commentModel.userHasDownvoted;
                commentModel.downvoteCount = previousUpvoteCount;
                self.reRenderDownvotes(commentModel.id);
            };
        ...
        replyButtonClicked: function(ev) {
            ...
            var parentId = replyButton.parents('.comment').first().attr("id");
        ...
        createCommentingFieldElement: function(parentId, existingCommentId) {
            ...
            if(existingCommentId) {
                var saveButtonText = this.options.textFormatter(this.options.saveText);
                    var postButtonText=this.options.textFormatter(this.options.postasText);
            ...
            var saveButton = $('<span/>', {
                ...
                text: saveButtonText,
                'onclick':'kre_post_button(this)'
            });
            
    // post button
                var postButton = $('<span/>', {
                'class': saveButtonClass + ' postasanonymous save highlight-background',
                text: postButtonText,
                'onclick':'kre_post_button(this)'
            });
        ...
        createNavigationElement: function() {
            ...
            if(this.options.enableReplying || this.options.enableUpvoting) {
                ...
                if(this.options.enableReplying || this.options.enableDownvoting) {
                navigationWrapper.append(popular);
                dropdownNavigation.append(popular.clone());
            }
            if(this.options.enableAttachments) {
        ...
        createCommentElement: function(commentModel) {
            ...
            if(commentModel["comment_deleted"]==true){$(commentEl).addClass("kredeleted")}
             $(commentEl).append("<a href ='#"+commentModel.id+"'></a>")
            ...
            if($(commentEl).hasClass("kredeleted"))$(commentWrapper).children().remove();
        ...
        createCommentWrapperElement: function(commentModel) {
            ...
            var time = $('<time/>', {
                ...
                'datetime': commentModel.created
            });

            // Name
           /* var nameText = commentModel.createdByCurrentUser ? this.options.textFormatter(this.options.youText) : commentModel.fullname;*/
          var nameText = commentModel.createdByCurrentUser ? this.options.textFormatter(commentModel.fullname==="Anonymous" ?(commentModel.fullname + " ("+this.options.youText+")"):this.options.youText) : commentModel.fullname;
            var name = $('<div/>', {
                'class': 'name'
            });
            ...
            var report_parent=$('<span/>',{
            	'class':'kre_report',
            	'onclick':'kre_report_click(this)'
            })
            // Report
            var report = $('<button/>', {
                'class': 'action report',
              ...
              var hashtag = $('<button/>', {
                'class': 'action hashtag',
                'type': 'button',
                text: this.options.hashtag
            });
report=report_parent.append(report);
            // Upvote icon
            var upvoteIcon = $('<i/>', {
...
if(this.options.enableUpvoting) actions.append(Downvotes);
if(this.options.enableUpvoting) actions.prepend(report);
 actions.append(hashtag);
 if($(".upvote",actions).hasClass("highlight-font")){
 	$(".Downvote",actions).attr("disabled",true)
 }else if($(".Downvote",actions).hasClass("highlight-font")){
 	$(".upvote",actions).attr("disabled",true)
 }else{
 	$(".Downvote",actions).attr("disabled",false);
 		$(".upvote",actions).attr("disabled",false);
 }
            if(commentModel.createdByCurrentUser || this.options.currentUserIsAdmin) {
                ...
                // Case: edit button for regular comment
                    ...
                    actions.find(".hashtag").before(editButton);
            ...
            wrapper.append(content);
             var div_create="<div class='kre_commentuser'></div>";
                       var div_append_act=$(div_create).append(time).append(name).append(actions)     
                  var kre_commentfollow="<span class='kre_commentfollow'></span>";
                              kre_commentfollow=$(kre_commentfollow).append(profilePicture).append("<button class='krefollow'>"+this.options.follow+"</button>")                                                                                                                          
            commentWrapper.append(kre_commentfollow).append(div_append_act).append(wrapper);
        ...
        createUpvoteElement: function(commentModel) {
            ...
            // Upvotes
            var upvoteEl = $('<button/>', {
                'class': 'action upvote krevote' + (commentModel.userHasUpvoted ? ' highlight-font' : ''),
                "onclick":"krevotebutton(this)",
                "data-vote":commentModel.userHasUpvoted,
...
createdownvoteElement: function(commentModel) {
            ...
            var downvoteEl = $('<button/>', {
                'class': 'action krevote Downvote' + (commentModel.userHasDownvoted ? ' highlight-font' : ''),
                "onclick":"krevotebutton(this)",
                "data-vote":commentModel.userHasDownvoted,
        ...
        reRenderDownvotes: function(id) {
            ...
            var self = this;
            commentElements.each(function(index, commentEl) {
                var doenvotes = self.createdownvoteElement(commentModel);
            ...
            commentElements.each(function(index, commentEl) {
                ...
                if($(upvotes).hasClass("highlight-font")){
                	$(commentEl).find('.Downvote').first().attr("disabled",true)
                }else{
                	 	$(commentEl).find('.Downvote').first().attr("disabled",false);
                	 	$(commentEl).find('.upvote').first().attr("disabled",false);
                }
            });
        },
     ...
     reRenderDownvotes: function(id) {
            ...
            commentElements.each(function(index, commentEl) {
                ...
                $(commentEl).find('.Downvote').first().replaceWith(Downvotes);
                 if($(Downvotes).hasClass("highlight-font")){
                	$(commentEl).find('.upvote').first().attr("disabled",true)
                }else{
                	 	$(commentEl).find('.Downvote').first().attr("disabled",false);
                	 	$(commentEl).find('.upvote').first().attr("disabled",false);
                }
